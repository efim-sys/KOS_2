namespace video {
    ...
    void main(void * arg) {
        ...
        KOS::onKeyPress(JOY_CENTER, [](uint8_t k){
            pause=!pause;
            if(pause and hasSound and KOS::soundPlayTask != NULL) {
    ...
    void init(fs::FS _fs = SD_MMC, String _filename = "/Video/CostaRica.VidBMP") {
        filesys = _fs;
        filename = _filename;

        

        xTaskCreate(main, "Video pl", 8192, videoPlTask, 10, NULL);
    }
}